...
ef  =  embedding_functions.OpenAIEmbeddingFunction(
            api_key=api_key,
            model_name= "text-embedding-3-large",  #"text-embedding-large",
...
def vectorize_documents(file_path, chroma_path, collection_name, distance_function):
    client = chromadb.PersistentClient(path=chroma_path)

    existing_collections = [col.name for col in client.list_collections()]
    if collection_name in existing_collections:
        print(f"Collection '{collection_name}' already exists. Getting the existing collection.")
        collection = client.get_collection(name=collection_name, embedding_function=ef)
    else:
        print(f"Creating new collection '{collection_name}' with distance function '{distance_function}'.")
        collection = client.create_collection(
            name=collection_name,
            embedding_function=ef,
            metadata={"hnsw:space": distance_function}  # Specify the distance function
        )
